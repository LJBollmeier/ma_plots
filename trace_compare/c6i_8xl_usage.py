...
base_dirs = ["trace_compare/c6i_8xlarge_sf100_instances4_repetitions10_new/", "trace_compare/c6in_large_sf100_instances50_repetitions10/"]
...
for nt, ct, worker_dir, xmin, xmax in zip(network_traces, cpu_traces, worker_dirs,  xmins, xmaxs):
    with open(ct) as fd:
        df = pd.read_csv(fd)
    instance_start = min(df["Time"])
...
fig.set_figwidth(4)
fig.tight_layout()
plot.savefig('c6i_8xl_usage.png', dpi=300)
plot.show()
