...
sustained_load_count = 12
part_length = int(duration/sustained_load_count)
for i in range(0, sustained_load_count):
    load = abs(math.sin(math.pi * 2 * (i + 0.5) / sustained_load_count) * max_cores)
    print(load)
    #load = int(math.ceil(rnd.random() * 10)) * max_cores/10
    for j in range(part_length * i, part_length * (i+1)):
        values[j] += load

burst_probability_per_second = 0.03
burst_duration = 2000 + int(rnd.random() * 3000)
...
for i in range(0, len(values)):
    if i > 5 * part_length and i < 8 * part_length:
        values[i] = 0
